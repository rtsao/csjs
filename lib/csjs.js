...
module.exports = function csjsHandler(strings) {
  ...
  var ignores = values.reduce((acc, val) => {
  ...
  var exports = Object.keys(hashes).reduce((acc, key) => {
    var val = ignores[hashes[key]] ? ignores[hashes[key]] : hashes[key];
    acc[val] = extensions[key] ? extensions[key] : makeComposition([key], [val], Boolean(hashes[key]));
    return acc;
  }, {});
